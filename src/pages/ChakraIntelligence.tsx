...
function ChakraStateCard({
  ...
  chakraLabel: string
  draftLevel: number
  setDraftLevel: (value: number) => void
  draftNote: string
  ...
  return (
    <div className={className}>
      ...
      <div className="text-sm text-white/85 mb-2">{chakraLabel}</div>
    ...
    <div className="min-h-screen bg-black text-white p-4 md:p-8 font-sans selection:bg-purple-500/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:h-[calc(100vh-140px)]">
        ...
        <motion.div 
          ...
          className="lg:col-span-4 lg:h-full flex flex-col gap-6"
        >
          <div className="lg:flex-1 bg-gradient-to-b from-purple-900/10 to-transparent rounded-2xl border border-white/5 p-1 relative overflow-hidden group">
            ...
            <ChakraStateCard
              lang={lang === 'ru' ? 'ru' : 'en'}
              chakraLabel={lang === 'ru' ? state.chakras[activeChakra].nameRu : state.chakras[activeChakra].name}
              draftLevel={draftLevel}
              setDraftLevel={setDraftLevel}
              draftNote={draftNote}
              setDraftNote={setDraftNote}
              onSave={() => updateChakra(activeChakra, { level: draftLevel, note: draftNote })}
              onReset={() => {
                setDraftLevel(state.chakras[activeChakra].level)
                setDraftNote((state.chakras[activeChakra] as any).note || '')
              }}
              className="hidden md:block absolute bottom-3 right-3 z-20 w-[240px] rounded-xl bg-neutral-900 border border-white/20 p-3 shadow-2xl"
            />
          </div>
          <ChakraStateCard
            lang={lang === 'ru' ? 'ru' : 'en'}
            ...
            className="md:hidden rounded-2xl bg-neutral-900 border border-white/20 p-4 shadow-2xl"
          />
        </motion.div>
        ...
        <motion.div 
          ...
          className="lg:col-span-5 lg:h-full flex flex-col gap-6"
        >
          <div className="lg:h-[45%]">
