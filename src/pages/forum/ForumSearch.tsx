...
export function ForumSearch({ onNavigate, initialQuery = '' }: Props) {
  ...
  const handleSearch = useCallback(async (q: string) => {
    ...
    setSearched(true)
    try {
      // Search in posts content (Supabase)
      ...
      const matchingPosts = (allPosts as ForumPost[])
        .filter(p => p.content.toLowerCase().includes(lowerQ))
        .slice(0, 20)

      // Also search topic titles (Supabase)
      ...
      for (const post of matchingPosts) {
        if (!seenTopics.has(post.topicId)) {
          const topic = (allTopics as ForumTopic[]).find(t => t.id === post.topicId)
          if (topic) {
