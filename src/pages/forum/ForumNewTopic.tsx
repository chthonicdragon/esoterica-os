...
export function ForumNewTopic({ user, categoryId, onNavigate, onSuccess }: Props) {
  ...
  const { addTopic: addLocalTopic } = useForumStorage()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error: topicError } = await supabase.from('forumTopics').insert([
        {
          id: topicId,
          categoryId,
          userId: user.id,
      ...
      // Create first post in Supabase
      const { error: postError } = await supabase.from('forumPosts').insert([
        {
          id: postId,
          topicId,
          userId: user.id,
