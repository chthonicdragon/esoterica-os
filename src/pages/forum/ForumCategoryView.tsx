...
export function ForumCategoryView({ user, categoryId, onNavigate }: Props) {
  ...
  const { getCategories: getLocalCategories, getTopics: getLocalTopics } = useForumStorage()
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const topicsWithAuthors = await Promise.all(
        (rawTopics as ForumTopic[]).map(async (topic) => {
          try {
            ...
            return {
              ...topic,
              authorName: profile?.displayName || topic.userId.slice(0, 8),
            }
          } catch {
      ...
      const localTopics = getLocalTopics(categoryId)
      setTopics(localTopics as ForumTopic[])
      setUsingLocalStorage(true)
    } finally {
      setLoading(false)
    }
  }, [categoryId, page, getLocalCategories, getLocalTopics])
      ...
      {usingLocalStorage && (
        <div className="mb-4 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center gap-2">
          ...
          <span className="text-sm text-amber-600 dark:text-amber-400">
            {lang === 'ru' 
              ? 'Локальное хранилище: темы работают в режиме браузера'
              : 'Local Mode: Topics run in browser storage'}
          </span>
        </div>
      )}
